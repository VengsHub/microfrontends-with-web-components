...
import {distinctUntilChanged, finalize, merge, shareReplay, startWith, Subject, switchMap, withLatestFrom} from 'rxjs';
...
export class WidgetsService {
  ...
  private readonly currentDashboardWidgets$ = this.updateTriggers$.pipe(
    ...
    switchMap(([_trigger, dashboardTab]) =>
      this.restWidgetsService.getDashboardWidgets(dashboardTab).pipe(startWith([]))
    )
  );
  ...
  private readonly updateDashboardWidgets$ = this.saveTrigger$.pipe(
    ...
    switchMap(([updatedWidgets, dashboardTab]) =>
      this.restWidgetsService.save(updatedWidgets, dashboardTab).pipe(startWith([]))
    ),
    finalize(() => this.alertService.successToast('Konfiguration gespeichert'))
  );

  readonly dashboardWidgets$ = merge(this.currentDashboardWidgets$, this.updateDashboardWidgets$).pipe(
  ...
  constructor(private readonly restWidgetsService: RestWidgetsService,
